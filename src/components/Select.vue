...
function intersectionFilterCards() {
  ...
  if (cards.getFilterRooms.length)
    result = result.filter((c) => cards.getFilterRooms.includes(c))
  if (cards.getFilterFloor.length)
    result = result.filter((c) => cards.getFilterFloor.includes(c))
  if (cards.getFilterSquare.length)
    result = result.filter((c) => cards.getFilterSquare.includes(c))
  if (cards.getFilterPrice.length)
    result = result.filter((c) => cards.getFilterPrice.includes(c))
...
onMounted(() => {
  ...
  createSlider(
    'square-slider',
    [minSquare.value, maxSquare.value],
    { min: 22.94, max: 192.27 },
    ([min, max]: Array<number>) => {
      minSquare.value = min
      maxSquare.value = max
      filterSquare()
